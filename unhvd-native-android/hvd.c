...
struct hvd *hvd_init(const struct hvd_config *config)
{
	...
	*/
	...
	LOGI("About to create hwdevice ctx: hwtype=%s device=%s hwpixfmt=%s", av_hwdevice_get_type_name(hardware_type), device, av_get_pix_fmt_name(h->hw_pix_fmt));
	if ((err = av_hwdevice_ctx_create(&h->hw_device_ctx, hardware_type, device, NULL, 0)) < 0)
	{
		LOGI("ffmpeg err=%s", av_err2str(err));
...
static enum AVPixelFormat hvd_find_pixel_fmt_by_hw_type(const enum AVHWDeviceType type)
{
	...
	case AV_HWDEVICE_TYPE_MEDIACODEC:
		fmt = AV_PIX_FMT_MEDIACODEC;  //AV_PIX_FMT_YUV420P; 
		break;
...
static enum AVPixelFormat hvd_get_hw_pix_format(AVCodecContext *ctx, const enum AVPixelFormat *pix_fmts)
{
	...
	LOGI("hvd: failed to get HW surface format");
...
AVFrame *hvd_receive_frame(struct hvd *h, int *error)
{
	...
	*error = HVD_ERROR;
	...
	{
		LOGI("hvd: unable to transfer data to system memory - \"%s\"", av_err2str(ret));
...
static void hvd_dump_sw_pix_formats(struct hvd *h)
{
	...
	{
		LOGI("hvd: failed to get transfer formats");
		return;
	}
	iterator=formats;

	LOGI("hvd: make sure you are using supported software pixel format:");
